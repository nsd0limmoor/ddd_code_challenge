...
class RaumController {
  ...
  RaumDto createRoom(@RequestBody final RaumDto raum) {
    ...
    return new RaumDto(created.getNummer().getValue(), created.getName().getValue(), personDtos);
  }
  ...
  RaumDto findRoom(@PathVariable final UUID id) {
    ...
    return new RaumDto(raum.getNummer().getValue(), raum.getName().getValue(), personDtos);
  }
